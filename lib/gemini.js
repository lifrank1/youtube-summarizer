...
// Debug logging
const DEBUG = true;
const log = (...args) => DEBUG && console.log('[YT-AI Gemini]', ...args);
const logError = (...args) => console.error('[YT-AI Gemini ERROR]', ...args);
...
async function callGemini(apiKey, prompt, history = []) {
  ...
  log('  Total content parts:', contents.length);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logError('API error response:', JSON.stringify(error, null, 2));
    ...
    if (!data.candidates || data.candidates.length === 0) {
      logError('No candidates in response:', JSON.stringify(data, null, 2));
      throw new Error('No response generated');
    }
    
    const result = data.candidates[0].content.parts[0].text;
    log('  Response received, length:', result?.length, 'chars');

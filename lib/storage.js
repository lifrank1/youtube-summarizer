...
export async function clearVideoCache(videoId = null) {
  try {
    if (videoId) {
      // Clear specific video's cache and chat
      const result = await chrome.storage.local.get([STORAGE_KEYS.VIDEO_CACHE, STORAGE_KEYS.CHAT_HISTORY]);
      const cache = result[STORAGE_KEYS.VIDEO_CACHE] || {};
      const chatHistory = result[STORAGE_KEYS.CHAT_HISTORY] || {};
      delete cache[videoId];
      delete chatHistory[videoId];
      await chrome.storage.local.set({ 
        [STORAGE_KEYS.VIDEO_CACHE]: cache,
        [STORAGE_KEYS.CHAT_HISTORY]: chatHistory
      });
    } else {
      // Clear all cache and chat history
      await chrome.storage.local.remove([STORAGE_KEYS.VIDEO_CACHE, STORAGE_KEYS.CHAT_HISTORY]);
    }
    return true;
  } catch (error) {
